...
public class TileEntityFiberizer extends TileEntity implements ISidedInventory, IEnergyReceiver
{
	...
	public void updateEntity()
	{
		...
		if(!this.worldObj.isRemote)
		{
			...
			{
				this.cookTime++;
				//storage.extractEnergy((int)((float)takenEnergyPerTick*currentMachineInformation.efficiency()), false);
			...
			{
				flag1 = true;
				BlockFiberizer.updateFiberizerBlockState(this.burnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
	...
	private boolean canSmelt()
	{
		...
		{
			ItemStack itemstack = FiberizerRegistry.getSmeltingResultItemStack(this.slots[0].getItem());
	...
	{
		return this.isInvNameLocalized() ? this.localizedName : "container.fiberizer";
	}
	...
	public int receiveEnergy(ForgeDirection from, int amount, boolean simulated) 
	{
		...
		return meta < 6 ? 0 :  meta < 10 ? storage.receiveEnergy(amount, simulated) : ((TileEntityFiberizer)worldObj.getTileEntity(xCoord, yCoord-1, zCoord)).storage.receiveEnergy(amount, simulated);
	}
	
	@Override
	public boolean isFull()
	{
		return worldObj.getBlockMetadata(xCoord, yCoord, zCoord) < 10 ? storage.isFull() : ((TileEntityFiberizer)worldObj.getTileEntity(xCoord, yCoord-1, zCoord)).storage.isFull();
	}
}
