...
public class TileEntityIndustrialFurnace extends TileEntity implements ISidedInventory, IEnergyReceiver
{
	...
	public int furnaceSpeed = 30;
	public final int defaultFurnaceSpeed = furnaceSpeed;
	...
	private EnergyStorage storage = new EnergyStorage(0, 355, 0, 0);
	
	private int takenEnergyPerTick = 6;
	...
	public void updateEntity()
	{
		...
		if(!this.worldObj.isRemote)
		{
			...
			{
				flag1 = true;
				BlockIndustrialFurnace.updateIndustrialFurnaceBlockState(this.burnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
	...
	private boolean canSmelt()
	{
		...
		{
			ItemStack itemstack = IndustrialFurnaceRegistry.getSmeltingResultItemStack(this.slots[0].getItem());
	...
	public void smeltItem()
	{
		if(canSmelt())
		{
			...
			ItemStack randomResult = IndustrialFurnaceRegistry.getSmeltingRecipe(this.slots[0].getItem()).chooseRandomAlternativeOutcome();
			tryInsertRandomResultItemStack(randomResult);
	...
	void tryInsertRandomResultItemStack(ItemStack itemStack)
	{
		if(itemStack == null)
		{
			return;
		...
		for(int i = 6; i < 12; i++)
		{
			if(this.slots[i] != null)
			{
				if(this.slots[i].isItemEqual(itemStack))
				{
					if(itemStack.stackSize + this.slots[i].stackSize < 64)
					{
						this.slots[i].stackSize += itemStack.stackSize;
					...
					else
					{
						...
						itemStack.stackSize -= possibleItems;
					}
				}
			}
			else
	...
	{
		return this.isInvNameLocalized() ? this.localizedName : "container.industrial_furnace";
	}
