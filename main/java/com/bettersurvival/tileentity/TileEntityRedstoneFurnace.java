...
public class TileEntityRedstoneFurnace extends TileEntity implements ISidedInventory, IEnergyReceiver
{
	private String localizedName;
	
	private static final int[] slots_top = new int[]{0};
	private static final int[] slots_bottom = new int[]{1};
	private static final int[] slots_sides = new int[]{0,2,3,4,5};
	...
	private int takenEnergyPerTick = 1;
	...
	public boolean canInsertItem(int slot, ItemStack stack, int side) 
	{
		if(slot == 0)
		{
			if(side > 0)
			{
				if(FurnaceRecipes.smelting().getSmeltingResult(stack) != null)
				{
					...
					else if(stack.stackSize + this.slots[0].stackSize <= 64 && stack.getItem() == this.slots[0].getItem())
					{
						return true;
					}
				}
		...
		else
		{
			if(side == 2)
			{
				if(EnergyUpgradeRegistry.isItemStackUpgrade(stack))
				{
					if(this.slots[slot] == null)
					{
						return true;
					}
					else if(stack.stackSize + this.slots[slot].stackSize <= 2 && stack.getItem() == this.slots[slot].getItem())
					{
						return true;
					}
				}
	...
	{
		return slot != 0 && side == 0 ? true : false;
	}
	
	public String getInvName()
	{
		return this.isInvNameLocalized() ? this.localizedName : "container.redstone_furnace";
	}
