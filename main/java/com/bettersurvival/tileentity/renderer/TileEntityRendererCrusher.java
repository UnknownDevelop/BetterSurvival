...
{
	private ModelCrusher model; // yup, this refers to the last file. so you'll get an error for now. it says what model it has to take. you could also make it a boat, or a cow. nice for statues or something. have it render the model of a cow.

	private ResourceLocation texture = new ResourceLocation("bettersurvival:textures/models/ModelCrusher.png");
	
	public TileEntityRendererCrusher()
	{
		model = new ModelCrusher();
	} // where and what to render

	public void renderAModelAt(TileEntityCrusher tile, double d, double d1, double d2, float f)
	{
	...
	{
		renderAModelAt((TileEntityCrusher) tileentity, d, d1, d2, f); //where to render
	}
}
