...
public class TileEntityQuartzFurnace extends TileEntity implements ISidedInventory
{
	...
	public static final float consumptionMultiplier = 0.55f;
	...
	public void tryItemRefill()
	{
		...
		if(this.slots[1] == null)
		{
			for(int i = 3; i < 11; i++)
			{
				if(this.slots[i] != null)
				{
					int itemBurnTime = getItemBurnTime(this.slots[i]);
					
					if(itemBurnTime != 0)
					{
						this.slots[1] = this.slots[i].copy();
						this.slots[i].stackSize = 0;
						this.slots[i] = null;
						break;
					}
		...
		if(this.slots[2] != null)
		{
			if(hasRefilledSlot0 || cantRefillSlot0)
			{
				for(int i = 11; i < 18; i++)
				{
					if(this.slots[i] != null)
					{
						if(this.slots[i].isItemEqual(this.slots[2]))
						{
							if(this.slots[2].stackSize + this.slots[i].stackSize < 64)
							{
								this.slots[i].stackSize += this.slots[2].stackSize;
							...
							else
							{
								...
								this.slots[2].stackSize -= possibleItems;
							}
						}
					}
					else
					{
						this.slots[i] = this.slots[2].copy();
						this.slots[2].stackSize = 0;
						this.slots[2] = null;
						break;
					}
	...
	{
		return this.isInvNameLocalized() ? this.localizedName : "container.quartz_furnace";
	}
