...
public class TileEntitySolarPanelSecondGen extends TileEntity implements IEnergyProvider
{
	public final int MAX_RECEIVE_PER_TICK = 5;
	public final int MAX_TRANSMIT_PER_TICK = 6;
	public final float DESERT_EFFICIENCY_MULTIPLIER = 1.7f;
	public final float HOT_EFFICIENCY_MULTIPLIER = 1.45f;
	
	public EnergyStorage storage = new EnergyStorage(0, 900, 0, MAX_TRANSMIT_PER_TICK);
	...
	public void updateEntity()
	{
		if(!this.worldObj.isRemote)
		{
			...
			BlockSolarPanelSecondGen.updateSolarPanelBlockState(isWorking(), this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			...
			if(Config.INSTANCE.sendWailaPackets())
			{
				...
				for(int i = 0; i < players.size(); i++)
				{
					...
					{
						BetterSurvival.network.sendTo(new PacketUpdateSolarPanelSecondGen(xCoord, yCoord, zCoord, storage.getEnergyStored(), working), (EntityPlayerMP) players.get(i));
					}
				}
			}
		}
