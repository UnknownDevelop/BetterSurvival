...
public class BlockCableColored extends BlockFacadeableContainer
{
	int color;
	
	float pixel = 1F/16F;
	
	public BlockCableColored(int color) 
	{
		super(Material.ground);
		
		this.color = color;
		
		setBlockName("block_cable_" + (color == ColorList.BLACK ? "black" : color == ColorList.RED ? "red" : color == ColorList.GREEN ? "green" : color == ColorList.BROWN ? "brown" : color == ColorList.BLUE ? "blue" : color == ColorList.PURPLE ? "purple" : color == ColorList.CYAN ? "cyan" : color == ColorList.LIGHT_GREY ? "light_gray" : color == ColorList.GREY ? "gray" : color == ColorList.PINK ? "pink" : color == ColorList.LIGHT_GREEN ? "light_green" : color == ColorList.YELLOW ? "yellow" : color == ColorList.LIGHT_BLUE ? "light_blue" : color == ColorList.MAGENTA ? "magenta" : color == ColorList.ORANGE ? "orange" : "white"));
	...
	{
		TileEntityCableColored cable = (TileEntityCableColored) world.getTileEntity(x, y, z);
	...
	{
		return new TileEntityCableColored().setColor(color);
	}
	...
	{
		return ((TileEntityCableColored)world.getTileEntity(x, y, z)).calculateRedstoneComparatorOutput();
	}
