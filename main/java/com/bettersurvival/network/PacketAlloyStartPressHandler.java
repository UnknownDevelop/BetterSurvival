package com.bettersurvival.network;

import net.minecraft.tileentity.TileEntity;
import net.minecraft.world.WorldServer;

import com.bettersurvival.tileentity.TileEntityAlloyPress;

import cpw.mods.fml.common.network.simpleimpl.IMessage;
import cpw.mods.fml.common.network.simpleimpl.IMessageHandler;
import cpw.mods.fml.common.network.simpleimpl.MessageContext;

public class PacketAlloyStartPressHandler implements IMessageHandler<PacketAlloyStartPress, IMessage> 
{
	public PacketAlloyStartPressHandler() {}
	
	@Override
	public IMessage onMessage(PacketAlloyStartPress message, MessageContext ctx) 
	{
		int xCoord = message.xCoord;
		int yCoord = message.yCoord;
		int zCoord = message.zCoord;
		WorldServer world = (WorldServer)ctx.getServerHandler().playerEntity.worldObj;
		TileEntity tileentity = world.getTileEntity(xCoord, yCoord, zCoord);
		
		if(tileentity instanceof TileEntityAlloyPress)
		{
			TileEntityAlloyPress tile = (TileEntityAlloyPress)tileentity;
			
			if(!tile.isPressing && tile.canPress())
			{
				tile.startPressing();
			}
		}
		
		return null;
	}
}
