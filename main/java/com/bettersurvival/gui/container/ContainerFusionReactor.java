...
{
	private TileEntityFusionReactor tileentity;
	
	public ContainerFusionReactor(InventoryPlayer inv, TileEntityFusionReactor tileentity2)
	{
		...
		for(int i = 0; i < 2; i++)
		{
			for(int j = 0; j < 9; j++)
			{
				this.addSlotToContainer(new Slot(tileentity2, j+i*9, 8+j*18, 16+i*18));
			}
		}
		
		for(int i = 0; i < 3; i++)
		{
			this.addSlotToContainer(new Slot(tileentity2, 18+i, 62+i*18, 57));
		}
		...
		for(int i = 0; i < 3; i++)
		{
			for(int j = 0; j < 9; j++)
			{
				this.addSlotToContainer(new Slot(inv, j+i*9+9, 8+j*18, 87+i*18));
			}
		}
		
		for(int i = 0; i < 9; i++)
		{
			this.addSlotToContainer(new Slot(inv, i, 8+i*18, 145));
		}
	}
